...
export const AdminRoutes = () => {
  ...
  return getUserDispatched && accountState.user?.role === "ADMIN" ? (
    <Outlet />
  ) : accountState.user?._id ? (
    <Navigate to={UrlConstants.HOME} state={{ from: location }} replace />
  ) : (
    <Navigate to={UrlConstants.LOGIN} state={{ from: location }} />
  );
};
