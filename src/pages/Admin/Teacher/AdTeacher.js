...
export const AdTeacher = () => {
  const locale = "vn";

  const dispatch = useDispatch();

  const teachers = useSelector((state) => state.teachers.items);
  const totalItems = useSelector((state) => state.teachers.total);
  ...
  useEffect(() => {
    dispatch(loadTeachersPaging(searchKeyword, currentPage));
  ...
  const handleDelete = () => {
    if (selectedItems) {
      swal({
        ...
        if (willDelete) {
          dispatch(deleteTeachers(selectedItems));
          setSelectedItems([]);
        }
      });
    }
  ...
  const teacherElements = teachers.map((teacher) => {
    return (
      <tr
        key={`teacher_${teacher._id}`}
        className={`table-row ${
          selectedItems.indexOf(teacher._id) !== -1 ? "selected" : ""
        }`}
        onClick={() => handleSelectRow(teacher._id)}
      >
        <td>
          <input
            type="checkbox"
            value={`${teacher._id}`}
            onChange={() => handleSelectRow(teacher._id)}
            checked={selectedItems.indexOf(teacher._id) !== -1}
          />
        </td>
        <td>{teacher._id}</td>
        <td>{teacher.name}</td>
        <td>{handleDate(teacher.birthday, locale)}</td>
        <td>{teacher.major}</td>
        <td>{teacher.email}</td>
        <td>{teacher.phone}</td>
        <td>
          <Link to={`edit/${teacher._id}`}>Sửa</Link>
        </td>
      </tr>
    );
  });
  ...
  return (
    <Fragment>
      <div>
        <h2 className="h3 mb-2 text-gray-800">Quản lý giảng viên</h2>
        {showSearch && (
